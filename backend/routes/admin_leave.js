...
router.get("/:email", async (req, res) => {
  ...
  router.get("/approve/:id", async (req, res) => {
    try {
      const leaveApplications = await leave.findOneAndUpdate({ leaveID: req.params.id },{status: "approved"},{new: true});
      res.json({ leaveApplications });
    } catch (err) {
      res.status(500).json({ message: err.message });
    }
  ...
  router.get("/approved/:id", async (req, res) => {
    try {
      const leaveApplications = await leave.find({ status: "approved",email: req.params.id });
      res.json({ leaveApplications });
    } catch (err) {
      res.status(500).json({ message: err.message });
    }
